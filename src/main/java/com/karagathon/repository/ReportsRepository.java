...
public interface ReportsRepository extends JpaRepository<Report, Long> {
	...
	@Query(value = "SELECT months.month as month, COUNT(r.date_of_report) as value" + "            FROM"
			+ "            (" + "						SELECT 1 AS MONTH"
			+ "                       UNION SELECT 2 AS MONTH" + "                       UNION SELECT 3 AS MONTH"
			+ "                       UNION SELECT 4 AS MONTH" + "                       UNION SELECT 5 AS MONTH"
			+ "                       UNION SELECT 6 AS MONTH" + "                       UNION SELECT 7 AS MONTH"
			+ "                       UNION SELECT 8 AS MONTH" + "                       UNION SELECT 9 AS MONTH"
			+ "                       UNION SELECT 10 AS MONTH" + "                       UNION SELECT 11 AS MONTH"
			+ "                       UNION SELECT 12 AS MONTH ) as months"
			+ "			LEFT JOIN report r ON months.month = MONTH(r.date_of_report)"
			+ "			AND YEAR(r.date_of_report) = :year" + "            GROUP BY months.month", nativeQuery = true)
	List<ModelStatistics> getMonthStatisticsFromReport(@Param("year") String year);

	@Query("SELECT COUNT(r) FROM Report r where r.dateOfReport = :currentDate")
	int getCountOfReports(@Param("currentDate") Date currentDate);
